...
def main():
    ...
    model_name="Top-K Masking Rv2,152"
    ...
    #Makes it a bit easier to restart training
        ...
        base_model=tf.keras.applications.resnet_v2.ResNet152V2(include_top=False,input_shape= (image_size, image_size, 3),weights=None)(inputs)
        
        #Adding a classification head
        flatten=tf.keras.layers.Flatten()(base_model)
        classification=tf.keras.layers.Dense(100,activation='softmax')(flatten)
        
        #Compiling the model
        Model = tf.keras.Model(inputs, classification)
