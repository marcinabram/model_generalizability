...
def main():
    ...
    model_name="Top-K Cut Rv2,50 cifar100-2"
    ...
    for epoch_count in range(starting_epoch,number_of_epochs):
        ...
        print(epoch_count,"   ",Model.percent)
        ...
        if((fit_metrics.history['Top 1'][0]>history[1]) and fit_metrics.history['Top 1'][0]>.2): #and (fit_metrics.history['Top 1']>previous_accuracy or fit_metrics.history['Top 1']>.99)):
            Model.percent+=.1*(.6-Model.percent)
...
class Masked_Model(tf.keras.Model):
    ...
    def train_step(self, next_batch):
        ...
        Data=mask_data(self.image_size,Data,Labels,self,self.percent)
...
def mask_data(image_size,Data,Labels,Model,percent):
    ...
    k=int(percent*3*image_size*image_size)
